...
static void BNO080_sendCalibrateCommand(BNO080_tbl* p_sensor, uint8_t thingToCalibrate);
static void BNO080_sendCommand(BNO080_Packet_tbl* p_packet, uint8_t command);
//=============================================================================
// Init / Open
//=============================================================================
...
bool BNO080_Calibration(BNO080_tbl* p_bno080, uint8_t isCaliOk)
{
  bool ret=false;
  bool readInfo=false;
  static bool caliStep_1=false;
  static bool isInitialized = false;
  bool caliStep_2=false;
  if(isCaliOk==1)
  {
    ...
    if(!isInitialized)
    {
      ...
      BNO080_calibrateAll(p_bno080);
      BNO080_enableGameRotationVector(p_bno080, 20000);
      BNO080_enableMagnetometer(p_bno080, 20000);
      printf("Calibrating BNO080...\n");
    ...
    if(readInfo)
    {
      ...
      float x = BNO080_getValue(p_bno080, rawMag, AccelX);
      float y = BNO080_getValue(p_bno080, rawMag, AccelY);
      float z = BNO080_getValue(p_bno080, rawMag, AccelZ);
      unsigned char accuracy = p_bno080->BNO080_Raw[rawMag].accuracy;

      float quatI =  BNO080_getValue(p_bno080, rawQuat, QuatI);
      float quatJ = BNO080_getValue(p_bno080, rawQuat, QuatJ);
      float quatK = BNO080_getValue(p_bno080, rawQuat, QuatK);
      float quatReal = BNO080_getValue(p_bno080, rawQuat, QuatReal);
      unsigned char sensorAccuracy = p_bno080->BNO080_Quat.quatAccuracy;
  ...
  if((caliStep_1==true) && (isCaliOk == 2))
  {
    BNO080_saveCalibration(p_bno080);
    BNO080_requestCalibrationStatus(p_bno080);
    ...
    while(1)
    {
      ...
      if(BNO080_ReadInfo(p_bno080))
      {
        if(BNO080_calibrationComplete(p_bno080))
        {
          ret=true;
...
void BNO080_calibrateAll(BNO080_tbl* p_bno080)
{
  BNO080_sendCalibrateCommand(p_bno080, CALIBRATE_ACCEL_GYRO_MAG);
}
...
{
  BNO080_Packet_tbl* p_packet = &p_sensor->BNO080_Packet;
  BNO080_setFeatureCommand(p_packet, SENSOR_REPORTID_MAGNETIC_FIELD, timeBetweenReports, 0);
}
